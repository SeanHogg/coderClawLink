...
| Method | Path | Description |
|--------|------|-------------|
| `POST` | `/marketplace/auth/register` | Register marketplace account (`{ email, username, password, display_name? }`) |
| `POST` | `/marketplace/auth/login` | Login (`{ email, password }`) → JWT |
| `GET` | `/marketplace/auth/me` | Current user profile (marketplace JWT required) |
...
| `PUT` | `/marketplace/users/me` | Update own profile (`{ display_name?, bio?, avatar_url? }`) |

### Skills

| Method | Path | Description |
|--------|------|-------------|
| `GET` | `/marketplace/skills` | List published skills (`?category=&q=&page=1&limit=24`) |
| `GET` | `/marketplace/skills/:slug` | Get skill detail (auto-increments download counter) |
...
```json
{
  ...
  "slug": "ts-linter",
  "description": "Runs ESLint + type-check on changed files",
  "category": "code-quality",
  "tags": ["typescript", "eslint"],
  "version": "1.0.0",
  "readme": "# TypeScript Linter\n\n...",
  "repo_url": "https://github.com/example/ts-linter"
}
```
...
| `GET` | `/api/admin/newsletter/subscribers` | Newsletter subscriber list |
| `POST` | `/api/admin/newsletter/send` | Send newsletter to subscribers |
| `GET` | `/api/admin/legal/documents` | List versioned legal documents |
| `POST` | `/api/admin/legal/documents` | Publish a new legal document version |
| `GET` | `/api/admin/privacy-requests` | List GDPR/CCPA privacy requests |
| `PATCH` | `/api/admin/privacy-requests/:id` | Update privacy request status |
