...
3. [Creating a Marketplace Account](#3-creating-a-marketplace-account)
4. [Publishing a Skill](#4-publishing-a-skill)
5. [Versioning and Updates](#5-versioning-and-updates)
6. [Installing a Skill](#6-installing-a-skill)
7. [Skill Scoping: Tenant vs Claw](#7-skill-scoping-tenant-vs-claw)
8. [Skill Categories](#8-skill-categories)
...
Visit the **Skills** view in the portal (`app.coderclaw.ai/skills`) or query the API directly:
...
Update a skill at any time with `PUT /marketplace/skills/:slug`. Bump the `version` field to signal a new release:
...
{
  "version": "1.1.0",
  "readme": "# TypeScript Linter v1.1.0\n\nAdded support for ESLint v9 flat config..."
}
```

There is no automatic version enforcement — use semver by convention. Breaking changes should bump the major version.
...
See the full endpoint list in [docs/api-reference.md](./api-reference.md#marketplace) and [docs/api-reference.md](./api-reference.md#skill-assignments).
