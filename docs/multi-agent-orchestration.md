...
1. [What is a Claw?](#1-what-is-a-claw)
2. [Agent Roles](#2-agent-roles)
3. [Registering Claws](#3-registering-claws)
4. [Skill Assignment and Capability Routing](#4-skill-assignment-and-capability-routing)
5. [Submitting Tasks for Execution](#5-submitting-tasks-for-execution)
6. [Execution Lifecycle](#6-execution-lifecycle)
7. [Real-Time Streaming](#7-real-time-streaming)
8. [Claw-to-Claw Delegation](#8-claw-to-claw-delegation)
9. [Spec-Driven Workflows](#9-spec-driven-workflows)
10. [Human-in-the-Loop Approvals](#10-human-in-the-loop-approvals)
11. [Fleet Discovery and Capability-Based Routing](#11-fleet-discovery-and-capability-based-routing)
12. [Example: Full Multi-Agent Pipeline](#12-example-full-multi-agent-pipeline)
...
The `/spec` TUI command in coderClaw generates a full planning document and pushes it to the portal:

```http
POST /api/specs
Content-Type: application/json
?clawId=7&key=clw_abc...xyz
...
The spec is visible in the portal under **Specs**. A manager reviews and approves it (`PATCH /api/specs/:id { status: "approved" }`), then the claw registers an execution workflow:
...

{
  ...
  "description": "JWT refresh token rotation — approved spec"
}
```

Workflow tasks are added (`POST /api/workflows/:id/tasks`) and per-task states updated (`PATCH /api/workflows/:id/tasks/:tid`) as the claw works through the plan. The portal shows the live DAG status.
  ...
  "claws": [
    {
      "id": 7,
      "name": "prod-claw-01",
      "capabilities": ["typescript", "eslint", "pr-review"],
      "connectedAt": "2026-03-04T12:00:00Z",
      "projects": [{ "id": "proj_abc", "name": "Main API" }]
    },
    {
      "id": 9,
      "name": "test-claw-01",
      "capabilities": ["jest", "vitest", "coverage"],
      "connectedAt": "2026-03-04T11:55:00Z",
      "projects": []
    }
  ]
}
...
Here is a complete pipeline: a PR is opened → code review claw reviews → test claw generates tests → human approves → deploy claw deploys.

```
┌──────────────────────────────────────────────────────────┐
│  PR opened (GitHub webhook → your orchestration script)  │
└───────────────────────────┬──────────────────────────────┘
                            │  POST /api/runtime/executions
                            ▼  (taskId: "review-pr-123", clawId: 7)
                   ┌────────────────┐
                   │  Review Claw   │  analyses diff, posts review
                   │  (claw-01)     │  → POST /api/claws/7/relay-result
                   └───────┬────────┘    (remote.result to claw-01)
                           │
                           │  POST /api/claws/9/forward (remote.task: "generate tests")
                           ▼
                   ┌────────────────┐
                   │  Test Claw     │  generates Jest tests
                   │  (claw-02)     │  → POST /api/claws/9/relay-result
                   └───────┬────────┘
                           │
                           │  POST /api/claws/7/approval-request
                           ▼  (actionType: "git_push", description: "Push to main")
                   ┌────────────────┐
                   │  Approval gate │  MANAGER reviews in portal
                   │  (human)       │  → PATCH /api/approvals/:id {approved}
                   └───────┬────────┘
                           │
                           ▼  approval.decision frame → claw-01
                   ┌────────────────┐
                   │  Deploy Claw   │  pushes branch, triggers CI
                   │  (claw-03)     │  → PATCH /api/runtime/executions/:id/state {completed}
                   └────────────────┘
```

All execution states, approval decisions, and code-change telemetry are recorded in the portal's immutable audit trail. The portal's real-time dashboard shows live status for every step.
